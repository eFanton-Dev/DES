...
#ifdef TESTING
    friend class DESTest;
#endif

private:
    KeySchedule* key;

    const size_t perm_table[64] = {
    ...
    const size_t inverse_perm_table[64] {
    ...
    std::string encrypt_PCBC(std::string msg, std::string iv);
    std::string decrypt_PCBC(std::string msg, std::string iv);
